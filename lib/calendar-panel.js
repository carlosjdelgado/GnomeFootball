...
// A grid-cell label that never wraps or ellipsizes, so a tight grid can't shrink
// team codes to "C…".
function noWrapLabel(text, { xAlign = Clutter.ActorAlign.CENTER, style = '' } = {}) {
...
export class CalendarPanel {
    ...
    _minuteLabel(match) {
        return noWrapLabel(liveMinute(match), { style: 'min-width: 36px; text-align: center;' });
    }
    ...
    _abbrevLabel(team, xAlign) {
        const label = noWrapLabel(teamAbbrev(team), { xAlign });
        if (this._tooltip)
    ...
    _scoreLabel(match) {
        return noWrapLabel(scoreOrTime(match),
