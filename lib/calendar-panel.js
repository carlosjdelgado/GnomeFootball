...
export class CalendarPanel {
    ...
    // Finished matches can't usefully be muted (there is nothing left to
    // notify), so the per-row bell and the "Mute all" action only cover
    // matches that haven't ended yet (state 'pre' or 'in').
    _muteableIds() {
    ...
    // A team's short acronym (e.g. "RSO", "BET", "UCH"). We use an acronym rather
    // than the full club name because the DateMenu column is narrow and long
    // names get truncated; teamAbbrev() also normalises ESPN's occasional long
    // abbreviations (e.g. "U. de Chile" → "UCH"). `xAlign` decides whether it
    ...
    _buildMuteButton(eventId) {
        ...
        const button = new St.Button({
            ...
            y_align: Clutter.ActorAlign.CENTER,
            child: new St.Icon({
                // The muted state is the bell-with-slash; its un-slashed sibling
                // ('notifications-symbolic') is not shipped by every Adwaita
                // build, so use the matching GNOME notifications bell, which is.
                icon_name: muted
