...
import { openMatchPage, canOpenMatch } from './match-link.js';
...
// Score cell styling; the link variant underlines on hover to signal it opens
// the match page (gated by the open-match-page-on-click setting).
const SCORE_STYLE = 'font-weight: bold;';
const SCORE_STYLE_LINK_HOVER = 'font-weight: bold; text-decoration: underline;';
...
function hasShootout(match) {
    return Number.isFinite(match.homeShootout) && Number.isFinite(match.awayShootout);
}
...
export class CalendarPanel {
    ...
    enable() {
        ...
        // Toggling click-to-open changes whether the score cell is a link, so
        // re-render the visible panel to add/remove the affordance.
        this._linkSettingSignalId = this._settings.connect(
            'changed::open-match-page-on-click', () => {
                if (this._section)
                    this._render();
            });
    ...
    disable() {
        ...
        if (this._linkSettingSignalId) {
            this._settings.disconnect(this._linkSettingSignalId);
            this._linkSettingSignalId = 0;
        }
    ...
    // The centre cell. When click-to-open is enabled and the match has a real
    // ESPN page, the score becomes a link that opens the match in the browser.
    // On knockout ties decided by penalties, the shootout "(X - Y)" goes on a
    // second small line under the score; both lines live inside a vertical box
    // so each line is independently x-centred in the grid column.
    _scoreLabel(match) {
        const score = noWrapLabel(scoreOrTime(match), { style: SCORE_STYLE });
        if (!hasShootout(match)) {
            if (this._settings.get_boolean('open-match-page-on-click') && canOpenMatch(match.eventId))
                this._makeScoreLink(score, match.eventId);
            return score;
        }
        const tail = noWrapLabel(`(${match.homeShootout} - ${match.awayShootout})`, {
            style: 'font-weight: bold; font-size: 0.7em;',
        });
    ...
    _makeScoreLink(label, eventId) {
        label.reactive = true;
        label.track_hover = true;
        label.connect('button-press-event', () => {
            openMatchPage(eventId);
            return Clutter.EVENT_STOP;
        });
        label.connect('notify::hover', () => {
            label.style = label.hover ? SCORE_STYLE_LINK_HOVER : SCORE_STYLE;
        });
        if (this._tooltip)
            this._tooltip.attach(label, gettext('Open on ESPN'));
    }
