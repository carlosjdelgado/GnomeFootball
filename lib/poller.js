// Periodic poller that queries ESPN for subscribed leagues, runs the event
// detector over each live (or about-to-start) match, and dispatches notifications.
...
import { fetchScoreboard, fetchSummary, disposeSession } from './espn-api.js';
import { detectEvents, pruneLiveState } from './event-detector.js';
import { isReplayActive } from './replay.js';
import { notifyEvent } from './notifier.js';
import { LIVE_STATE_FILE, readJsonSync, writeJsonSync } from './storage.js';
    ...
    enable() {
        ...
        const forceHandler = this._settings.connect(
            ...
            () => this._runTickAsync().catch(e =>
                console.warn(`[GnomeFootball] forced tick failed: ${e.message}`)
            )
        );
    ...
    async _runTickAsync() {
        ...
        try {
            ...
            if (subscribedSlugs.length === 0) {
                console.log('[GnomeFootball] tick: no subscriptions, skipping');
                return;
            }

            const enabledEvents = this._readEnabledEvents();
            const liveState = readJsonSync(LIVE_STATE_FILE, {});
            ...
            for (const slug of subscribedSlugs) {
                if (this._cancellable.is_cancelled())
                    return;
                const counts = await this._processLeague(slug, subscriptions[slug], enabledEvents, liveState);
                tracked += counts.tracked;
                notified += counts.notified;
            }

            const pruned = pruneLiveState(liveState);
            writeJsonSync(LIVE_STATE_FILE, pruned);

            const elapsedMs = Date.now() - startMs;
            console.log(`[GnomeFootball] tick: ${subscribedSlugs.length} leagues queried, ${tracked} matches tracked, ${notified} notifications sent (${elapsedMs} ms)`);
        } finally {
            this._cancellable = null;
            this._running = false;
        }
