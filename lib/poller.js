...
import { toMatchView, compareMatchViews, localDateKey, matchPassesSubscription } from './match-model.js';
...
export class Poller {
    constructor(settings, mute = null) {
        ...
        // Read-only snapshot of the matches in each subscribed league's current
        // scoreboard, keyed by slug. Refreshed every tick and consumed by the
        // calendar panel (Feature 1) for "today" without re-fetching.
        this._todayMatches = new Map();
        this._onUpdate = null;
        // Per-match mute controller (Feature 3): consulted before dispatching a
        // notification and told which matches have finished so it can auto-expire
        // their mutes. Null until F3 wires it in.
        this._mute = mute;
    }

    // Register a callback fired after each poll tick so the panel can refresh
    // its "today" view. Pass null to clear.
    setOnUpdate(cb) {
        this._onUpdate = cb ?? null;
    }

    // Aggregated, sorted copy of today's matches across all subscribed leagues,
    // filtered to the user's LOCAL today. Safe for the panel to consume — it is
    // a fresh array of plain view objects, not internal state.
    getTodayMatches() {
    ...
    async _runTickAsync() {
        ...
        try {
            ...
            // Expire mutes for matches that have reached full-time (and age out
            // any stragglers). A POST match keeps its liveState entry for a while,
            // but afterTick is idempotent, so re-passing it does no harm.
            if (this._mute) {
    ...
    async _processLeague(slug, subscription, enabledEvents, liveState, reminderLeadMinutes, preWindowMinutes) {
        ...
        // Snapshot for the calendar panel: every subscribed match in this
        // scoreboard (NOT gated by _isMatchRelevant — the panel lists all of
        // the day's matches, including pre-match ones far from kickoff).
        ...
        for (const scoreboardEvent of events) {
            ...
            // detection stays consistent: when the user un-mutes mid-match we
            // resume from "now" rather than replaying everything missed.
