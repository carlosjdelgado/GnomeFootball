...
// Transient per-match mute state (Feature 3). Kept separate from live-state.json
// so the poller's wholesale rewrite of that file each tick can never clobber a
// mute toggled between ticks (by a notification action or the panel). Entries
// auto-expire (at full-time and by age), so this file is strictly throwaway
// runtime state — never anything the user needs to manage.
