// League and team catalog: loads team rosters from ESPN, normalizes the response,
// caches the result in GSettings (catalog-cache-json) for CATALOG_TTL_SECONDS.
...
import { fetchScoreboard, fetchTeams } from './espn-api.js';
...
function extractTeamsFromTeamsResponse(response) {
    // ESPN /teams response: sports[0].leagues[0].teams = [{ team: {...} }, ...]
...
export async function refreshCatalog(settings, cancellable = null) {
    const catalog = {};
