...
export async function fetchScoreboard(slug, cancellable = null) {
    const localPath = GLib.build_filenamev([FIXTURES_DIR, slug, 'scoreboard.json']);
    const local = await tryReadLocalFixture(localPath);
    if (local)
        return local;
    const url = `${API_BASE_URL}/${encodeURIComponent(slug)}/scoreboard`;
    return fetchJson(url, cancellable);
}

// Scoreboard for a specific date range (YYYYMMDD or YYYYMMDD-YYYYMMDD), used by
// the calendar panel for past/future days. The caller passes a day±1 range and
// filters client-side by local date (ESPN's `dates` param is UTC). Disk-first
// off the SAME fixtures/<slug>/scoreboard.json the poller reads: this lets the
// e2e runner drive every day (past/today/future) from a single fixture whose
// events carry the relevant dates, since each consumer filters by local day.
// Only the network fallback uses the date range.
export async function fetchScoreboardForDate(slug, dateRange, cancellable = null) {
    const localPath = GLib.build_filenamev([FIXTURES_DIR, slug, 'scoreboard.json']);
    const local = await tryReadLocalFixture(localPath);
    if (local)
        return local;
    const url = `${API_BASE_URL}/${encodeURIComponent(slug)}/scoreboard?dates=${encodeURIComponent(dateRange)}`;
    return fetchJson(url, cancellable);
}

export async function fetchSummary(slug, eventId, cancellable = null) {
    const localPath = GLib.build_filenamev([FIXTURES_DIR, slug, `summary-${eventId}.json`]);
    const local = await tryReadLocalFixture(localPath);
    if (local)
        return local;
    const url = `${API_BASE_URL}/${encodeURIComponent(slug)}/summary?event=${encodeURIComponent(eventId)}`;
    return fetchJson(url, cancellable);
}

export async function fetchTeams(slug, cancellable = null) {
    const localPath = GLib.build_filenamev([FIXTURES_DIR, slug, 'teams.json']);
    const local = await tryReadLocalFixture(localPath);
    if (local)
        return local;
    const url = `${API_BASE_URL}/${encodeURIComponent(slug)}/teams`;
    return fetchJson(url, cancellable);
}
