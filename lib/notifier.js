...
import { openMatchPage as launchMatchPage } from './match-link.js';

let _source = null;
let _extensionPath = null;
let _defaultGicon = null;
let _settings = null;
let _muteController = null;

export function initNotifier(extensionPath, settings, muteController = null) {
    _extensionPath = extensionPath;
    _settings = settings ?? null;
    _muteController = muteController ?? null;
    _defaultGicon = null;
}
...
function formatTitleAndBody(event) {
    ...
    // Append shootout aggregate when ESPN exposes it (knockout ties).
    const hasShootout = Number.isFinite(event.homeShootout) && Number.isFinite(event.awayShootout);
    const score = hasShootout
        ? `${event.homeScore}-${event.awayScore} (${event.homeShootout}-${event.awayShootout})`
...
// Open the ESPN match page in the user's default browser, respecting the user's
// preference. Read live so toggling in prefs takes effect immediately.
function openMatchPage(event) {
    if (_settings && !_settings.get_boolean('open-match-page-on-click'))
        return;
    launchMatchPage(event.eventId);
}
...
export async function notifyEvent(event) {
    ...
    // "Mute match" action: silences further notifications for this match (until
    // full-time) and dismisses the current card.
    if (_muteController) {
        try {
            notification.addAction(gettext('Mute match'), () => {
                _muteController.mute(event.eventId);
                notification.destroy();
            });
        } catch (e) {
            console.warn(`[GnomeFootball] could not add mute action: ${e.message}`);
        }
    }
